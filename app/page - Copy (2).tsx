...
export default function Home() {
  ...
  async function deleteItem(itemId: string, itemName: string) {
    const pass = prompt(`Enter Management Password to DELETE "${itemName}":`);
    if (pass !== process.env.NEXT_PUBLIC_DELETE_PASSWORD) {
      alert('Unauthorized! Incorrect management password.');
      return;
    }
    if (!confirm(`WARNING: This will permanently delete "${itemName}" and ALL its transaction history. Proceed?`)) return;

    const { error: transError } = await supabase.from('transactions').delete().eq('item_id', itemId);
    if (transError) {
      alert('Error clearing history: ' + transError.message);
      return;
    }
    ...
    if (!itemError) {
      ...
      alert('Error deleting item: ' + itemError.message);
    }
  }
  ...
  async function exportReport() {
    ...
    const { data: allHistory } = await supabase.from('transactions').select(`id, qty, type, created_at, item_id`).order('created_at', { ascending: true });
    ...
    sheet1.addRow(h1); sheet1.addRow(h2);
    ...
    allItems.forEach((item: any, idx: number) => {
      const itemTrans = allHistory.filter((t: any) => t.item_id === item.id);
      const preMonthTrans = itemTrans.filter((t: any) => t.created_at < startOfMonth);
      const withinMonthTrans = itemTrans.filter((t: any) => t.created_at >= startOfMonth && t.created_at <= endOfMonth);
      
      let initialStock = 0;
      if (preMonthTrans.length > 0) {
        initialStock = preMonthTrans.reduce((acc: number, t: any) => acc + t.qty, 0);
      } else if (withinMonthTrans.length > 0) {
        initialStock = withinMonthTrans[0].qty;
      }

      const netInMonth = withinMonthTrans.reduce((acc: number, t: any) => acc + t.qty, 0);
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        const dayTrans = withinMonthTrans.filter((t: any) => t.created_at.startsWith(dStr));
        const isFirstDayOfNewItem = preMonthTrans.length === 0 && dayTrans.length > 0 && dayTrans[0].id === withinMonthTrans[0]?.id;
        
        row.push(
          dayTrans.filter((t: any, i: number) => t.type === 'in' && !(isFirstDayOfNewItem && i === 0)).reduce((s: number, t: any) => s + t.qty, 0) || 0,
          dayTrans.filter((t: any) => t.type === 'out' || (t.type === 'adjustment' && t.qty < 0)).reduce((s: number, t: any) => s + Math.abs(t.qty), 0) || 0
        );
      }
      const newRow = sheet1.addRow(row);
      if (finalStock <= 0) newRow.getCell(4).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFC0CB' } };
    });

    const sheet2 = workbook.addWorksheet('Adjustment Report');
    sheet2.addRow(['Date', 'Item Name', 'Difference Found', 'Status']);
    const monthAdj = allHistory.filter((t: any) => t.type === 'adjustment' && t.created_at >= startOfMonth && t.created_at <= endOfMonth);
    
    monthAdj.forEach((t: any) => {
      const itemName = allItems.find((i: any) => i.id === t.item_id)?.name || 'Unknown';
        ...
        {items.filter(i => i.name.toLowerCase().includes(search.toLowerCase())).map((item: any) => (
          <div key={item.id} className={`bg-white border p-3 rounded-lg flex justify-between items-center ${item.stock === 0 ? 'border-red-500 bg-red-50' : ''}`}>
            ...
            <div className="flex gap-2 items-center">
              ...
              <button onClick={() => deleteItem(item.id, item.name)} className="ml-2 text-gray-300 hover:text-red-500">
